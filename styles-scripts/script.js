...
bnt_form_cadastro.addEventListener('click', async function name(event) {
    ...
    // ENVIAR OBJETO PARA O BACK-END =========================================
    if(form.reportValidity()){
        const objeto = {
            nameUser: id_name.value,
            emailUser: id_email.value,
            passwordUser: senha_usuario
        };

        console.log(objeto.passwordUser);
        try{
            const fetch_url = await fetch("http://localhost:1200/api/createUser", {
                method: 'POST',
                headers:{
